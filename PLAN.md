...
- [ ] Create **Clerk** account → get Publishable key, Secret key, Webhook signing secret
...
## Phase 3 — Auth (Clerk, Google only)

- [ ] Confirm `@clerk/expo` supports **Expo SDK 57**, then install + pin it
- [ ] Install `expo-secure-store`
- [ ] Wrap app in `ClerkProvider` (token cache) in `src/app/_layout.tsx`
- [ ] Create an **Android development build** (`npx expo run:android`)
- [ ] Configure Google OAuth (Clerk dashboard + Google Cloud + Clerk Native Applications page)
- [ ] **Verify:** Google sign-in works on the Android dev build
...
- [ ] Pin the confirmed `@clerk/expo` version
